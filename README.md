...
Google, Bing, OpenAI, Gemini, Perplexity, and other provider calls are deliberately not implemented. IndexNow has a standalone, hard-gated executor described below; it is not enabled by application startup or deployment. Existing Shopify agents/LLM infrastructure remains unchanged.
...
`PROCESSING` queue rows use their non-null `claimedAt` timestamp as the ownership token for that processing attempt. Completion and failure must present the exact token; delayed workers lose ownership without mutating a later claim. Expired processing leases are recovered explicitly in bounded batches, either returning work to its pending slot or skipping it when a newer pending successor exists. The hard-gated IndexNow executor creates `IndexAttempt` audit rows only for actual provider invocations and uses the model's per-queue-item unique attempt number. Production execution remains off unless the standalone worker is explicitly configured and run; materialization remains unwired, and HTTP, webhook, scan, dashboard, startup, authentication, and migration paths do not execute providers. Google product push does not exist.
...
An already-produced provider plan can now be materialized atomically into provider-specific pending queue rows, but this preparation helper remains unwired. `INTERNAL` stays the durable source intent and its lifecycle is not changed. For each target, only that provider's opposite pending action is cancelled before the existing enqueue semantics run. Materialization creates no provider call or `IndexAttempt`, requires no credentials, and does not implement IndexNow keys or domain verification; those concerns are intentionally deferred to provider-execution work.

IndexNow ownership-verification readiness is modeled defensively in a pure gate. A root key file can cover the whole matching host; a non-root key file covers only its path scope. Consequently Shopify app-proxy paths such as `/apps/` and `/tools/` do not verify normal `/products/` URLs. For the Robotto deployment specifically, manual production verification proved that the same-host root key URL redirect to Shopify-hosted plain text is accepted by IndexNow. This is an empirical deployment fact for `www.robotto.se`, not a generic assumption for other Shopify shops.

The standalone IndexNow executor processes only already-materialized `INDEXNOW` queue rows and is off unless `INDEXNOW_EXECUTION_ENABLED=true` exactly. When enabled it additionally requires an injected `INDEXNOW_KEY` and `INDEXNOW_KEY_LOCATION`; the key remains outside source control. It claims one queue item at a time by default, requires ROOT readiness, creates one `IndexAttempt` per actual request, and preserves exact `claimedAt` ownership for completion or retry/failure. HTTP 200 and 202 complete work; 429, server, network, and timeout failures use bounded retry backoff; terminal client and ownership failures do not retry. Merging or deploying the code does not enable the worker, and no HTTP, webhook, scan, dashboard, startup, authentication, or migration path invokes it.

Provider materialization remains deliberately unwired, so existing `INTERNAL` backlog rows are never converted or consumed automatically. There is no Google product-push implementation. No `write_app_proxy` or other Shopify scope was added.
